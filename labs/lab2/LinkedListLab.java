...
public class LinkedListLab<AnyType> implements Iterable<AnyType> {
    ...
    public AnyType kthToLast(int k) {
        ...
        if (k < 1){
            return null; //return null for less than 1.
        }

        Node<AnyType> firstPointer = head;
        Node<AnyType> secondPointer = head;

        for (int i = 0;  i < k; i++) {
            if (firstPointer == null) {
                return null; //k being greater than list length.
            }
            firstPointer = firstPointer.next;
        }

        //if secondPointer moved with first now, first has already cover k steps.
        while (firstPointer != null) {
            firstPointer = firstPointer.next;
            secondPointer = secondPointer.next;
        }
        //so the data returned should be k steps from last for the secondPointer.
